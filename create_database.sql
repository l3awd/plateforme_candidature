...
INSERT INTO
    Centre (nom, adresse, ville, telephone, email)
VALUES
    (
        'Centre Rabat',
...
INSERT INTO
    Specialite (nom, code, description)
VALUES
    (
        'Informatique',
        'INFO',
...
INSERT INTO
    Utilisateur (nom, prenom, email, mot_de_passe, role, centre_id)
VALUES
    -- Administrateur
    (
        'Admin',
...
SELECT
    ...
    SUM(
        CASE
            WHEN cand.etat = 'Validee' THEN 1
            ELSE 0
        END
    ) as candidatures_validees,
    SUM(
        CASE
            WHEN cand.etat = 'Rejetee' THEN 1
            ELSE 0
        END
    ) as candidatures_rejetees,
    SUM(
        CASE
            WHEN cand.etat = 'Soumise' THEN 1
            ELSE 0
        END
...
FROM
    ...
    LEFT JOIN Candidature cand ON ce.id = cand.centre_id
    AND s.id = cand.specialite_id
    AND co.id = cand.concours_id
GROUP BY
    ce.id,
    s.id,
    co.id;